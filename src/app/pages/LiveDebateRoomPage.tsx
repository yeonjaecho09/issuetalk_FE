...
import { getDebateRoomById, getLiveDebateRooms } from '../data/liveDebateRooms';
...
export function LiveDebateRoomPage() {
  ...
  const runtimeRooms = useMemo(() => getLiveDebateRooms().map(room => applyRuntimeToRoom(room)), [refreshKey]);
  ...
  const room = useMemo(() => (baseRoom ? applyRuntimeToRoom(baseRoom) : null), [baseRoom, refreshKey]);
  const joinedSide = useMemo(
    () => (room && currentUser ? getJoinedSide(room.id, currentUser.userId) : null),
    [room, currentUser, refreshKey],
  );
  const availableSides = useMemo(() => (room ? getAvailableSides(room) : []), [room, refreshKey]);
  const debateProgress = useMemo(() => (room ? getDebateProgress(room) : null), [room, clockTick]);
  const audienceQuestions = useMemo(() => (room ? getAudienceQuestions(room.id) : []), [room, refreshKey]);
  const currentVote = useMemo(() => (room ? getAudienceVote(room.id, actorId) : null), [room, actorId, refreshKey]);
  ...
  const handleSubmitAudienceQuestion = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
  ...
  const handleReport = () => {
    ...
    createAdminReport({
      ...
      reason: '사용자 신고',
      description: '토론 관전/참여 페이지에서 접수된 토론 신고입니다.',
